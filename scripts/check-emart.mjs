...
try {
  ...
  await ready("ko");
  check(await page.locator("h1").innerText() === "이마트 샘플점", "Fresh visit defaults to Korean");
  ...
  await ready("zh");
  check(await page.locator("h1").innerText() === "易买得示例店", "Chinese store name");
  check(await page.getByRole("button", { name: "2F", exact: true }).getAttribute("aria-pressed") === "true", "Language switch preserves selected floor");
  ...
  check(await page.locator("h1").innerText() === "易买得示例店", "Reload remembers selected language");
  await page.goto(`${origin}/`);
  await page.goto(`${origin}/event/emart/`); await ready("zh");
  check(await page.locator("h1").innerText() === "易买得示例店", "Return visit remembers selected language");
  await page.goto(`${origin}/event/emart/?lang=ko`); await ready("ko");
  check(await page.locator("h1").innerText() === "이마트 샘플점", "URL ko overrides stored zh");
  await page.getByRole("button", { name: "한국어", exact: true }).click();
  await page.goto(`${origin}/event/emart/?lang=zh`); await ready("zh");
  check(await page.locator("h1").innerText() === "易买得示例店", "URL zh overrides stored ko");
  await page.getByRole("button", { name: "한국어", exact: true }).click();
  ...
  for (const language of ["ko", "zh"]) {
    ...
    for (const width of [320, 390, 430]) {
      ...
      for (const floor of ["B2", "B1", "1F", "2F"]) {
    ...
    await page.getByRole("button", { name: language === "ko" ? /수유실/ : /母婴室/ }).click();
    check(await page.getByRole("button", { name: "1F", exact: true }).getAttribute("aria-pressed") === "true", `${language} facility jumps to correct floor`);
  }
  for (const route of ["/", "/pm/", "/PM/", "/PM/print/"]) {
    ...
    check(!(await page.locator("body").innerText()).includes("이마트 샘플점"), `Existing ${route} remains separate`);
  }
