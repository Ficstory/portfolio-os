...
완성된 PDF는 `관련 자료`에 document 링크로 추가한다. 읽기 화면에서 새 탭으로 여는 `PDF 보기`와 `다운로드` 링크를 제공한다.

```markdown
## 관련 자료

- document | final-report | [완성한 홍보기획서](/til/media/learning-note/final-report.pdf) | 최종 제출한 PDF 원본
```

PDF 원본도 사진·영상과 같은 미디어 폴더에 보관한다. URL에는 해당 글의 slug와 영문·숫자·하이픈·밑줄로 된 단일 파일명, `.pdf` 확장자를 사용한다. 하위 폴더·상대 경로·쿼리·다른 글의 slug는 허용하지 않는다. 검증은 원본 파일의 존재를 확인하며, 공개·비공개 전환과 웹 파일 생성도 사진·영상과 함께 처리한다. 기존 외부 문서 링크는 그대로 사용할 수 있다.
