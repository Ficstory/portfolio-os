...
2026-09-14 공개 자료 정리: 아래 기록의 원문 발췌 TXT는 사실 확인을 위한 보관 자료다. 채용 담당자가 읽을 완성된 산출물로 보기 어려워 페이지의 발췌 링크와 자료만 나열한 섹션을 제거했다. 개인 작업과 팀 기여는 본문에서 설명하며, 시연 영상과 실제 외부 공개 자료는 유지한다. 이 결정은 아래의 초기 발췌 링크 노출 방침보다 우선한다.

2026-09-11 전체 문장·근거 점검: PM, 공통 상세, OS 소개·역량·이력서, 공공디지털·정책·국회 트랙을 수정했다. 아래의 초기 기여·구성 설명보다 [전체 편집 점검 기록](./2026-09-11-editorial-audit.md)을 우선한다. 애낌의 팀원 구현·QA 기여와 개인 작업을 구분하고, 각 프로젝트 원본 문서·Git으로 확인한 범위를 보완했다.

2026-09-11 추가: 프로젝트 영상 미리보기의 원본·사용 구간·구현은 [화면 미리보기 기록](./media-previews.md)에 정리했다. 아래는 1차 구현 당시의 기록으로, 정적 이미지와 자동 재생 관련 설명은 추가 기록을 우선한다.

2026-09-11 Play Pick 추가: 원본 온보딩을 별도 로컬 환경에서 실행해 취향 선택부터 첫 추천까지 30.58초 영상으로 저장하고, 메인과 전용 상세에 반영했다. 상세에는 2인 팀에서 맡은 화면 설계·FE 구현·온보딩 API 연동과 PM 관점의 판단을 정리했다. [녹화·재현·기여 근거](./play-pick-demo/README.md)를 참고한다. 아래 Play Pick 자료 부족 표기는 1차 구현 당시의 상태다.
...
## 2026-09-11 애낌 앱 데모 녹화

애낌의 정지 화면 중심 홍보 영상 발췌를, 기존 Android 앱 UI에 데모 데이터를 연결해 직접 녹화한 26.75초 영상으로 교체했다. 결제 후보 확인부터 구독 등록·합계 갱신·상세 조회까지 포함한다. 캡처 6장과 재현 패치, 실제 앱과 데모의 경계는 [오프라인 앱 시연 기록](./aekkim-demo/README.md)에 정리했다. 실제 백엔드나 결제 데이터 수집이 작동하는 증거로 설명하지 않는다.
