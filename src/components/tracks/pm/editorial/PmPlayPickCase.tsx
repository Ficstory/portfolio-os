  ...
  return <main id="pm-content" tabIndex={-1} className={styles.container}>
    ...
    <div className={styles.caseLayout}>
      <nav className={styles.caseNav} aria-label="Play Pick 사례 목차">
        <a href="#context">상황과 문제</a><a href="#decision">온보딩 기준</a><a href="#implementation">화면과 데이터 연결</a><a href="#contribution">개인 기여</a><a href="#results">구현 결과</a>
      </nav>
      <div className={styles.caseContent}>
        ...
        <section id="implementation" className={styles.caseSection}>
          ...
          <details className={styles.evidenceDetails}><summary>구현 기준 자세히 보기</summary><div><p>보고싶어요는 1.5, 안볼래요는 −1.0으로 전달하며, 중립 응답은 원본 프런트엔드에서 저장 요청에 포함하지 않습니다. 서버는 공연 임베딩을 가중 합산하고 정규화해 선호 벡터로 저장합니다.</p><p>저장 실패 시 오류를 보여주고 완료 처리를 진행하지 않습니다. 이처럼 사용자 행동의 의미와 요청 순서, 실패 상태를 함께 다뤘습니다.</p></div></details>
        </section>
