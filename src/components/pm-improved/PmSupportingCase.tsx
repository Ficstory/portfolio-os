  ...
  return <>
    ...
    <CaseBody labels={{
      ...
      <CaseSection id="result" label="사례 3B / Failure UX" title="카메라 실패를 사용자가 이해할 수 있는 안내로 바꿨습니다.">
        ...
        <p>초대·대기·카메라 오류 사례는 화면·상태 구현과 명세까지 기록으로 확인됩니다. 초대 코드 만료·재연결·기기별 카메라 복구의 조건별 검증 기록은 확인되지 않습니다.</p>
        <figure className={detail.mediaEvidence}>
          <SmileTeamDemoVisual />
      ...
      <CaseSection id="learning" title="범위 변경 이유와 상태별 완료 조건을 남기겠습니다">
        <p>설문 분석에서는 친구 방과 안전 장치를 우선 제안했고, 최종 기능 범위에는 랜덤 매칭도 포함됐습니다. 다음에는 설문으로 검토할 제품 결정을 먼저 적고, 원래 가설·관찰·최종 범위·변경 이유를 하나의 기록으로 연결하겠습니다.</p>
        <p>초대 코드 보존·로그인 복귀와 대기실 참가자·Ready 상태를 구현했습니다. 다음에는 로그인 여부·코드 유효성·참여·재접속을 조합한 시나리오를 만들고, 이벤트 출처와 권한 주체를 상태 전이로 정의해 중복 이벤트와 순서 역전을 확인하겠습니다.</p>
        <p>카메라 권한·장치·보안 오류는 서로 다른 안내로 분류했습니다. 다음에는 오류별 원인 설명, 사용자가 취할 행동, 재시도 동작을 하나의 완료 조건으로 적고 기기·브라우저별 재현 결과를 기록하겠습니다.</p>
      </CaseSection>
    </CaseBody>
  </>;
}
