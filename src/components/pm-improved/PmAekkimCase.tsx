  ...
  return <main id="pm-content" tabIndex={-1} className={base.container}>
    ...
    <CaseBody labels={{
      ...
      <CaseSection id="evidence" label="사례 1 / 인증과 온보딩" title="로그인 제공자별 인증을 공통 세션·가입 흐름으로 연결했습니다.">
        ...
        <p>지금 다시 진행한다면 제공자별 정상·취소·만료 응답을 예시 데이터로 먼저 고정하고, 화면 구현 전에 계약 테스트를 작성하겠습니다.</p>
      </CaseSection>
      ...
      <CaseSection id="decision" label="사례 2 / 비동기 상태" title="성공과 실패 사이에 있는 중복, 재진입, 되돌리기를 화면 상태로 다뤘습니다.">
        ...
        <p>중복 제출·후보 재생성·알림 삭제 실패에 대응하는 상태 처리를 구현했습니다. 체크인의 일부는 시연 동작으로 확인했습니다. 다음에는 재진입·중복·복원을 기능별 완료 조건에 넣고 실행 결과를 남기겠습니다.</p>
      </CaseSection>
      ...
      <CaseSection id="action" label="사례 3 / 분석 파이프라인" title="서로 다른 분석 결과를 하나의 검토 가능한 구독 후보로 모았습니다.">
        ...
        <SubscriptionFlow />
        <p>절감 예상값은 가정과 상한을 적용한 추정치입니다. 다시 만든다면 분석 단계별 입력·출력과 실패 시 다음 경로를 짧은 결정 기록으로 남겨, 설계가 바뀌어도 판단 근거를 따라갈 수 있게 하겠습니다.</p>
      </CaseSection>
      ...
      <CaseSection id="collaboration" label="문서와 QA / Delivery" title="변하는 구현을 요구사항과 화면 명세에 다시 반영하고, 확인할 시나리오를 남겼습니다.">
        ...
        <p>온디바이스 응답 계약·대시보드 계산 테스트 코드와 QA 계획을 작성했습니다. QA 실행 결과표는 비어 있으며, 사용자 검증 기록은 확인되지 않습니다. 다음에는 계획과 실행 결과를 함께 남기겠습니다.</p>
      </CaseSection>
      ...
      <CaseSection id="learning" label="다음 개선 / Reflection" title="다음에는 통합 기준과 검증 증거를 기능보다 먼저 만들겠습니다.">
        <p>후반 수정의 많은 부분은 외부 로그인, 서버 응답, AI 결과처럼 서로 다른 경계에서 상태를 맞추는 일이었습니다. 다음 프로젝트에서는 요구사항마다 담당 범위, API 계약, 완료 조건, 검증 자료를 한곳에 두고, 경계 기능부터 결과 타입과 예시 데이터 기반 테스트를 작성하겠습니다.</p>
        <p>QA 계획에는 기기와 빌드, 실행 결과, 남은 제한을 함께 기록하고, 설계 초안과 최종 구조가 달라졌다면 짧은 결정 기록으로 이유를 남기겠습니다.</p>
      </CaseSection>
    </CaseBody>
